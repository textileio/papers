...
Even higher-level database abstractions could also be built on Threads. For example, a key/value store built on Threads would ``map'' key/value operations, such as \texttt{Put}, \texttt{Get}, and \texttt{Del} to an internal (i.e., private) view Model with similarly defined Event Creators and Resolver functions. These Events would then mutate the internal (map-like) view Model effectively encapsulating the entire Event Store in a database structure that satisfies a given interface (see Figure \ref{ex:KVStore} for example).
