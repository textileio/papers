...
\item New Events may trigger web-hooks\footnote{https://en.wikipedia.org/wiki/Webhook}, which could enable many complex (e.g., IFTTT\footnote{\url{https://ifttt.com}}) workflows.
\end{enumerate}
...
\end{example} \label{ex:DocStore}

Where \texttt{Indexable} can be satisfied by any structure with a \texttt{key} field and and \texttt{Query} is taken from the \texttt{go-datastore} interface library\footnote{https://github.com/ipfs/go-datastore} or similar.
