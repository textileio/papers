...
In multi-writer systems, determining causal order in all replicas is particularity challenging. As previously discussed (Section \ref{sec:merkleclocks}), a multi-writer solution, such as \texttt{ipfs-log}\footnote{\url{https://github.com/orbitdb/ipfs-log}}, creates a Merkle-Clock that is only partially ordered. That is, this approach cannot achieve a total order of events without implementing a data-layer conflict resolution strategy \cite{sanjuanMerkleCRDTs2019}:
...
Multi-writer systems based on linked log events [MISSING] with relaxed conssitency [CAP] (which is common in p2p systems) can lead to situtaitons in which merges are imppossible due to computational or network-based restrictions. For example, any one missing event may cause a branching or split, leading to (effectively) independent event 'trees'.
...
in multi-writer logs we solve this by having linked-data in single-writer logs

in multi-writer systems, conflicts arise along the way
in order to proceed, we need to define ways to deal with conflits
with imperfect information (which is common) this inevitably leads to situations where we need to decide how to reconsile the conflict
this might include a merge, etc

in single writer systems, conflict resolution is deferred to a point at which a decision is required
this means that imperfect information may be supplemented along the way, without causing conflucts
it also means that applications can choose conflict resolution strategies specific to the task at hand
if using crdt, conflicts are irrelevant
in some instances, an additional clock may be required, such as vector or bloom clock to deal with total ordering
finally, even merges are possible if this is the desired confluct resolution strategy
