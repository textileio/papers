...
The Dispatcher, combined with Actions and Creators provide a useful interface for the write side of the Threads CQRS framework. Like many CQRS-based systems, the read side revolves around the concept of views (or Stores). As Actions are dispatched on the write side, they are reacted to on the read side via Reducers, causing updates to Read/View Models, which provide interfaces for queries and accessing state. Read Stores are also responsible for notifying downstream consumers (application logic) of changes to their state via \emph{broadcast channels}. In practice, a Store is defined by a Reducer function, a set of query Resolver functions \cite{ereminReduxInspiredBackend2019}, and an Event Bus (or Broadcast channel). This similar in some respects to a Redux Store, though it is possible to have \emph{multiple} Stores as in the more general Flux pattern.

\begin{definition}
  (Reducer). Reducers are ``pure'' functions (where possible)\footnote{In practice they aren't really pure, because we are mutating (persistent) state here}, and are written to modify a Store's state based on incoming Actions.  
\end{definition}
...
A database built on Threads would map database operations into Actions. Here, the \emph{domain} is the databse. These Actions would then mutate a ReadModel via its corresponding Reducer function, as per above. The Store could be persisted or kept in-memory (as a ViewModel), and would be directly queried via corresponding database-style Resolver metods. Because Textile uses ES patterns, any delete-type operation would simply lead to an internal \emph{tombstone} Action, marking the database item for removal from the Store.
