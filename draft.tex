...
% To create tables + create checkmarks
\usepackage{booktabs}
\usepackage{colortbl}
...
% Thread ID components fig requrements
...
On the downside, and as discussed above (section \ref{sec:LogicalClocks}), a Merkle-Clock cannot order divergent heads (or roots) (see Figure \ref{fig:merkledag})
...
\caption{Example Access Control List.}
    \centering
    \begin{tabular} [t]{l>{\centering}p{0.2\linewidth}>{\centering}p{0.2\linewidth}>{\centering}p{0.2\linewidth}>{\centering\arraybackslash}p{0.2\linewidth}}
\toprule
...
Followers are represented as additional addresses, meaning that a Log address book may contain \emph{multiple} multiaddresses for a single Log (Example {ex:Multiaddress}).
...
A unique Thread IDentity (TID) is used to group together Logs which compose a single dataset and as a topic identifier within Pub/Sub-based synchronization. TIDs are defined with the following format {see Figure \ref{fig:ThreadID}
...
\phantom{Thread ID = }\phubar{0x62}{Multibase}%
  \phantom{\quad0x01\quad}\phubar{0x55}{Variant} \phantom{\quad7513b4dcf4efa291ffa92221659f19e2}
\par\vspace{-.6\baselineskip}\leavevmode
\label{fig:ThreadID}
\end{figure*}
...
\item \label{Perf2} Events can be pulled from replicating Followers via HTTP, libp2p, RSS, Atom, etc.
    a. In conjunction with push over WebSockets (2a above), this method provides web-based Readers and Followers with a reliable mechanism for receiving Log Events (Figure \ref{fig:Pulling}).
...
\end{lstlisting}
  \caption{Interfaces for an event body, header and node.}
