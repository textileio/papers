...
Our solution to deal with divergent Merkle-Clocks is to institute a single-writer rule: A log can only be updated by a single replica or identity. An Event Log is a single-writer Merkle-Clock that can be totally ordered (Figure \ref{fig:EventLog}). Separate Event Logs can be composed into advanced structures, including CRDTs [Ref Enes].
...
As seen above in Section \ref{sec:merkleclocks}., a Merkle-Clock is simply a Merkle-DAG of Events:
...
Followers are represented as additional addresses, meaning that a Log address book may contain multiple multiaddresses for a single Log (Example {ex:Multiaddress}).
