...
A key/value store built on Threads would ``map'' these database Events into key/value operations, such as \texttt{Put}, \texttt{Get}, and \texttt{Del}. These Events would then mutate a map-like View Model. To complete the database-like API, the entire API could be encapsulated in a database sturcture that satisfies a given interface (see for example, Figure \ref{ex:KVStore}).
